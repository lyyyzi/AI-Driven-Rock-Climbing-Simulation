...
class Joint():
    ...
    def rot_to_mjc_dof(self, rot):
        dof_shape = list(rot.shape[:-1])
        dof_shape = dof_shape + [4]
        dof = None

        if (self.joint_type == JointType.ROOT):
            pass
        elif (self.joint_type == JointType.HINGE) or (self.joint_type == JointType.SPHERICAL):
            dof = rot.roll(1)
        elif (self.joint_type == JointType.FIXED):
            pass
        else:
...
class KinCharModel():
    ...
    def load_char_file(self, char_file):
        ...
        # char_data = np.load(r'data\\char.npz')
        # char_body_names = char_data["body_names"]
        # char_parent_indices = char_data["parent_indices"]
        # char_local_translation = char_data["local_translation"]
        # char_local_rotation = char_data["local_rotation"]

        # remapped_body_indexes = np.where(body_names == char_body_names[:, None])[1]
        # char_joints = [joints[idx] for idx in remapped_body_indexes]
        # char_joints[0] = joints[0]
        # char_joints[1] = joints[1]
        # char_geoms = None
    ...
    def rot_to_mjc_dof(self, rot):
        ...
        for j in range(1, num_joints):
            ...
            if (j_dof_dim > 0):
                j_rot = rot[..., j - 1, :]
                j_dof = joint.rot_to_mjc_dof(j_rot)
                dof.append(j_dof)
