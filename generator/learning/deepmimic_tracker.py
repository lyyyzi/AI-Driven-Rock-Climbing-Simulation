...
class DMTracker:
    def __init__(self, num_segments, device):
        self._device = device
        self._episodes = 0
        ...
        self._ep_len_buf = torch.zeros([num_segments], device=device, dtype=torch.long)
        self._eps_per_env_buf = torch.zeros([num_segments], device=device, dtype=torch.long)
        #super().__init__(num_envs, device)

        # tracking cumulative rewards of all episodes starting in a segment
        self._return_bufs = dict()
        self._return_bufs["total_r"] = torch.zeros([num_segments], device=device, dtype=torch.float32)
        self._return_bufs["pose_r"] = torch.zeros([num_segments], device=device, dtype=torch.float32)
        self._return_bufs["vel_r"] = torch.zeros([num_segments], device=device, dtype=torch.float32)
        self._return_bufs["root_pos_r"] = torch.zeros([num_segments], device=device, dtype=torch.float32)
        self._return_bufs["root_vel_r"] = torch.zeros([num_segments], device=device, dtype=torch.float32)
        self._return_bufs["key_pos_r"] = torch.zeros([num_segments], device=device, dtype=torch.float32)
        
        # tracking number of fails of all episodes starting in a segment ## NOTE do not include end of motion as fail
        self._contact_fails = torch.zeros([num_segments], device=device, dtype=torch.int64)
        self._pose_fails = torch.zeros([num_segments], device=device, dtype=torch.int64)
        self._root_pos_fails = torch.zeros([num_segments], device=device, dtype=torch.int64)
        self._root_rot_fails = torch.zeros([num_segments], device=device, dtype=torch.int64)
