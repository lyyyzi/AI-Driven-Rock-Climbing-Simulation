...
class MDM(MotionGenerator):
    def __init__(self, cfg):
        ...
        self._use_hf_collision_loss = False
        self._use_hold_pos_loss = True

        self._ood_chance = cfg['ood_chance']
        if not self._use_hold_pos_loss:
            self._ood_chance = 0

        self._hold_body_idx = [self._kin_char_model.get_body_id(body) for body in ["right_hand", "left_hand", "right_foot", "right_foot"]]
    ...
    def normalize_holds_pos(self, holds_pos):
        return holds_pos / self._holds_pos_scale

    def unnormalize_holds_pos(self, holds_pos):
        return holds_pos * self._holds_pos_scale
    ...
    def assemble_mdm_features(self, ml_component_dict: dict, standardize=True):
        ...
        for comp_name in self._frame_components:
            ...
            elif comp_name == MDMFrameType.GRIPS:
                ml_grips = ml_component_dict[MDMFrameType.GRIPS]
                mdm_feature_list.append(ml_grips)
            else:
    ...
    def extract_motion_features(self, mdm_features: torch.Tensor, unstandardize=True):
        ...
        for i in range(len(self._frame_components)):
            ...
            if comp_name == MDMFrameType.ROOT_POS:
                ...
                grips = comp
                ml_component_dict[comp_name] = grips

        return ml_component_dict

    def compute_stats(self, sampler: MDMClimbMotionSampler, stats_filepath=None):
        ...
        mdm_features_mean = torch.zeros((self._seq_len, self._motion_frame_dof), dtype=torch.float32, device=self._device)
        ...
        mdm_features_std = torch.sqrt(mdm_features_std / (num_sequences - 1))

        # Certain features should not be standardized when they are very simple, like grips (1 or 0)
        if MDMFrameType.GRIPS in self._feature_slices:
            mdm_features_mean[:, self._feature_slices[MDMFrameType.GRIPS]] = 0.0
            mdm_features_std[:, self._feature_slices[MDMFrameType.GRIPS]] = 1.0
    ...
    def construct_conds(self, motion_data: dict, holds_pos: torch.Tensor, target_dir: torch.Tensor,
                        test: bool):
        ...
        if self._use_heightmap_obs:
            conds[MDMKeyType.OBS_KEY] = self.normalize_holds_pos(holds_pos)

        # TODO: use target_dir for climb
        if self._use_target_obs:
    ...
    def motion_difference(self, true_motion_data, pred_motion_data, conds, loss_fn):
        ...
        if self._use_simple_loss:
            ...
            if MDMFrameType.GRIPS in true_motion_data:
                grips_true = true_motion_data[MDMFrameType.GRIPS]
                grips_pred = pred_motion_data[MDMFrameType.GRIPS]
                grips_loss = loss_fn(grips_true, grips_pred)
                losses[LossType.GRIP_LOSS] = 1.0 * grips_loss
        ...
        if self._use_hold_pos_loss:
            # hold_pos -> (rh, lh, rf, lf)
            hold_pos = conds[MDMKeyType.OBS_KEY]
            target_hold = hold_pos[:, -1:]
            grips_pred = torch.clamp(pred_motion_data[MDMFrameType.GRIPS], min=0, max=1).round()

            hold_pos_loss = []

            for i, body_idx in enumerate(self._hold_body_idx):
                body_hold_pos = body_pos_pred[:, :, body_idx]
                dist1 = torch.linalg.norm(body_hold_pos - hold_pos[:, i].unsqueeze(1), dim=-1)
                dist2 = torch.linalg.norm(body_hold_pos - target_hold, dim=-1)
                dist = torch.minimum(dist1, dist2)
                dist_loss = 0.5 * torch.sum(torch.square(grips_pred[..., i] * dist), dim=-1)
                hold_pos_loss.append(dist_loss)

            losses[LossType.HOLD_POS_LOSS] = self._w_hf * (hold_pos_loss[0] + hold_pos_loss[1] + hold_pos_loss[2] + hold_pos_loss[3])
    ...
    def sample_motions(self, motion_sampler: MDMClimbMotionSampler, batch_size):
        motion_ids = motion_sampler._mlib.sample_motions(batch_size)
        start_times = motion_sampler._sample_motion_start_times(motion_ids, self._sequence_duration)

        motion_data, holds_pos, target_dir =\
            motion_sampler.sample_motion_data(motion_ids=motion_ids, motion_start_times=start_times)

        return motion_data, holds_pos, target_dir
    ...
    def sample_and_compute_losses(self, max_t, motion_sampler, batch_size, loss_fn,
        ...
        with torch.no_grad():
            motion_data, holds_pos, target_dir = self.sample_motions(motion_sampler, batch_size)

            conds, x_0 = self.construct_conds(motion_data, holds_pos, target_dir, test=test)
            ...
            if ood_batch_size > 0:
                # randomly offset target_holds
                rand_offset = torch.rand((ood_batch_size, 1, 3), device=self._device, dtype=torch.float32) * 0.3 - 0.15
                new_holds_pos = holds_pos[use_ood_loss, -1:] + rand_offset
                conds[MDMKeyType.OBS_KEY][use_ood_loss, -1:] = new_holds_pos
        ...
        if ood_batch_size > 0:
            for loss_type in losses:
                if loss_type == LossType.HOLD_POS_LOSS or loss_type == LossType.TARGET_LOSS:
                    continue
    ...
    def project_dofs(self, x):
        ...
        if MDMFrameType.GRIPS in self._frame_components:
            grips = motion_features[MDMFrameType.GRIPS]
            grips = torch.clamp(grips, min = 0.0, max = 1.0).round()
            motion_features[MDMFrameType.CONTACTS] = grips
        ...
        for epoch in range(self._epochs):
            ...
            if not test_only:
                if epoch % self._epochs_per_checkpoint == 0 and epoch > 0 and checkpoint_dir is not None:
                    checkpoint_save_path = checkpoint_dir / f"model_{epoch}.pkl"
    ...
    def gen_sequence(self, conds, ddim_stride=None, mode=GenerationMode.MODE_DDIM):
        ...
        if self._use_heightmap_obs:
            ...
            conds[MDMKeyType.OBS_KEY] = self.normalize_holds_pos(conds[MDMKeyType.OBS_KEY])
            pass
    ...
    def attention_debug(self, conds, ddim_stride=None, mode=GenerationMode.MODE_DDIM):
        ...
        final_sequence = self.gen_sequence(conds, ddim_stride, mode)
